...
const referenceOverlay = `{/* WON OVERLAY - Unified with Map style */}
                    {gameState === 'finished' && (
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    ...
                                    <div className="bg-white/5 backdrop-blur-md rounded-[2.5rem] border border-white/10 p-6 overflow-hidden text-center shadow-2xl flex flex-col items-center">
                                        ...
                                        <div className="w-full text-left">
                                            <ActivityRanking activityId={__ACT_ID__} limit={3} sortBy="score" />
                                        </div>
                                    </div>
                                    ...
                                    <div className="bg-white/5 backdrop-blur-md rounded-[2.5rem] border border-white/10 p-6 overflow-hidden text-center shadow-2xl flex flex-col items-center">
                                        ...
                                        <div className="w-full text-left">
                                            <ActivityRanking activityId={__ACT_ID__} limit={3} sortBy="time" />
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div className="flex flex-col md:flex-row gap-4 justify-center items-center max-w-5xl mx-auto w-full mt-2">
                                    <div className="w-full md:w-[calc(50%-8px+8px)] flex-none bg-slate-900/40 backdrop-blur-md rounded-2xl border border-white/10 p-0 shadow-xl overflow-hidden h-[120px] flex items-center justify-center">
                                        <div className="scale-[0.6] origin-center w-[166%] h-[166%] flex items-center justify-center -mt-8">
                                            <RatingSystem activityId={__ACT_ID__} />
                                        </div>
                                    </div>

                                    <button
                                        onClick={__RESET_FN__}
...
if (index !== -1) {
    ...
    if (content.includes('effectiveActivityId')) {
        newBlock = newBlock.replace(/__ACT_ID__/g, 'effectiveActivityId');
    } else {
        newBlock = newBlock.replace(/__ACT_ID__/g, 'activityId');
    }

    if (content.includes('const resetGame = () =>')) {
        newBlock = newBlock.replace(/__RESET_FN__/g, 'resetGame');
    } else {
        newBlock = newBlock.replace(/__RESET_FN__/g, '() => setGameState(\'start\')');
    }

    content = content.substring(0, index) + newBlock + content.substring(endIndex + 1);

...
import { RefreshCw, CheckCircle2, Trophy, ArrowRight, BookOpen, Volume2, Timer, Mic, Star } from 'lucide-react';
import { useLanguage } from '@/context/LanguageContext';
import confetti from 'canvas-confetti';
import { IRREGULAR_VERBS_BASIC, type IrregularVerb } from './data/irregular-verbs-basic';
...
export default function IrregularVerbsBasicGame({ taskId = null, type = 'writing' }: { taskId?: string | null, type?: 'writing' | 'pronunciation' }) {
    ...
    const {
        ...
        handleFinish
    } = useGameLogic({ initialTime: 90, penaltyTime: 0, gameMode, taskId }); // Less time for fewer verbs
    ...
    const startNewGame = (mode: 'challenge' | 'practice' = 'challenge') => {
        setGameMode(mode);
        const shuffled = [...IRREGULAR_VERBS_BASIC].sort(() => Math.random() - 0.5);
        setVerbs(mode === 'challenge' ? shuffled : shuffled.slice(0, 10)); // 10 for practice
        setCurrentIndex(0);
        setStreak(0);
        hookStartGame();
    ...
    const nextVerb = () => {
        ...
        if (gameMode === 'challenge') {
            if (currentIndex >= verbs.length - 1) {
                const reshuffled = [...IRREGULAR_VERBS_BASIC].sort(() => Math.random() - 0.5);
                setVerbs(reshuffled);
                setCurrentIndex(0);
            } else {
    ...
    return (
        <div className="w-full flex flex-col items-center select-none max-w-6xl mx-auto p-4">
            <GameHUD
                ...
                gameMode={gameMode}
                totalTargets={verbs.length || IRREGULAR_VERBS_BASIC.length}
                remainingTargets={(verbs.length || IRREGULAR_VERBS_BASIC.length) - currentIndex}
                targetName={currentVerb ? currentVerb.translation : ''}
                message={message}
                onReset={resetGame}
                colorTheme="purple"
                icon={<Star className="w-8 h-8 text-violet-500" />}
                title={type === 'pronunciation' ? "Verbos Básico (Voces)" : "Verbos Irregulares Básico"}
            />
                ...
                {gameState === 'start' && (
                    <div className="absolute inset-0 z-50 bg-black/60 backdrop-blur-md flex flex-col items-center justify-center p-8 text-center">
                        ...
                        <h2 className="text-3xl md:text-5xl font-black text-white mb-4 tracking-tight uppercase flex items-center gap-4">
                            Verbos Irregulares <span className="text-violet-500">BÁSICO</span>
                        </h2>
                        <p className="text-gray-300 mb-8 max-w-md text-lg leading-relaxed font-medium">
                            Comienza con los 25 verbos más usados del inglés. ¡La base de todo!
                        </p>
                        <div className="flex flex-col sm:flex-row gap-4 w-full justify-center">
                            ...
                            >
                                <span className="relative z-10 flex flex-col items-center gap-1">
                                    ...
                                    <span className="text-xs opacity-70 font-bold tracking-wider">25 VERBOS • 90 SEG</span>
                                </span>
                            </button>
                ...
                {gameState === 'finished' && (
                    <div className="absolute inset-0 z-50 bg-black/80 backdrop-blur-md flex flex-col items-center justify-center p-8 text-center animate-in fade-in duration-500">
                        ...
                        <h2 className="text-4xl font-bold text-white mb-2">
                            {gameMode === 'challenge' && timeLeft === 0 ? '¡Tiempo Agotado!' : '¡Nivel Básico Superado!'}
                        </h2>

                        <div className="flex flex-col items-center gap-2 mb-10 bg-white/5 p-8 rounded-3xl border border-white/10">
                            <span className="text-gray-400 text-xs uppercase tracking-[0.2em] font-bold">Puntuación</span>
                        ...
                        <AnimatePresence mode="wait">
                            {currentVerb && (
                                ...
                                >
                                    ...
                                    <p className="text-violet-50 text-xl font-medium px-4 py-2 bg-black/10 rounded-full">

...
import { MapPin, ZoomIn, ZoomOut, Maximize, Minimize, HelpCircle, RefreshCw, Timer, Trophy } from 'lucide-react';
            ...
            <div className={cn("w-full flex flex-col items-center", isFullscreen ? "max-w-6xl mx-auto p-6 min-h-screen justify-center" : "max-w-6xl mx-auto p-4")}>
                ...
                >
                        ...
                        {gameState === 'start' && (
                            <div className="absolute inset-0 z-30 bg-slate-900/60 backdrop-blur-xl flex flex-col items-center justify-start p-6 text-center rounded-[2rem] overflow-y-auto custom-scrollbar">
                                ...
                                <div className="flex flex-col items-center mb-8 shrink-0 mt-4">
                                    ...
                                    <p className="text-gray-400 max-w-xl text-lg leading-relaxed font-medium">{description}</p>
                                </div>
                                ...
                                <div className="w-full max-w-5xl flex flex-col gap-8 mb-10">
                                    ...
                                    <div className="text-center opacity-50 shrink-0">
                                        <p className="text-white text-xs font-black uppercase tracking-[0.3em]">
                                            {true ? '¡Prepárate para el éxito!' : 'Get ready for success!'}
                                        </p>
                                    </div>
                                </div>
                            </div>
                    ...
                    <div className={cn("absolute right-4 flex flex-col gap-2 z-20 transition-all duration-300", isFullscreen ? 'top-32 md:top-28' : 'top-4')} onMouseDown={e => e.stopPropagation()}>
                        ...
                        <button onClick={() => { setZoom(1); setPan({ x: 0, y: 0 }); }} className="p-2 bg-slate-800/80 text-white rounded-lg hover:bg-slate-700 backdrop-blur-sm border border-white/10 cursor-pointer" title="Reset View"><RefreshCwIconGame className="w-5 h-5" /></button>
                        <div className="h-2" />
                ...
                <p className="text-gray-500 text-xs mt-4 flex items-center gap-2">
                    ...
                    <span>{true ? 'Usa los controles o rueda del ratón para hacer zoom.' : 'Use controls or mouse wheel to zoom.'}</span>
                </p>
            </div>
        </div>
    );

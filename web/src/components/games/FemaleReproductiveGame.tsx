...
export default function FemaleReproductiveGame({ taskId = null, activityId }: { taskId?: string | null, activityId?: string }) {
    ...
    const effectiveActivityId = activityId || "sistema-reproductor-femenino";
    ...
    const labelPositions = React.useMemo(() => {
        const sortedByY = [...FEMALE_PARTS].sort((a, b) => a.y - b.y);
        const unmatchedSorted = FEMALE_PARTS
            .filter(p => !matches[p.id])
            .sort((a, b) => {
                const nameA = (t.gamesPage.femaleReproductive as any)[a.nameKey] || '';
                const nameB = (t.gamesPage.femaleReproductive as any)[b.nameKey] || '';
                return nameA.localeCompare(nameB, language);
            });

        return FEMALE_PARTS.map((part) => {
    ...
    return (
        <div className="w-full px-4 md:px-8 pb-4 md:pb-8 pt-0 select-none">
            <GameHUD
                ...
                gameMode={gameMode}
                totalTargets={FEMALE_PARTS.length}
                remainingTargets={FEMALE_PARTS.length - Object.keys(matches).length}
                targetName=""
                ...
                >
                    ...
                    >
                        ...
                        {Object.entries(matches).map(([labelId, partId]) => {
                            const part = FEMALE_PARTS.find(p => p.id === partId);
                            const label = labelPositions.find(p => p.id === labelId);
                            if (!part || !label) return null;
                            return (
                                <line
                                    key={`line-${labelId}`}
                                    x1={label.labelX > 500 ? label.labelX : label.labelX + 200}
                                    y1={label.labelY + 25}
                                    x2={part.x}
                                    y2={part.y}
                                    stroke="#4ade80"
